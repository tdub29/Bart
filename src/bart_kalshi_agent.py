    ...
    def list_events_for_date(
        ...
        *,
        ...
        for _ in range(max_pages):
            ...
            # Optionally stop early once we've paged past the target date (events often newest-first).
            # Disabled so we don't miss target date if API order differs; we filter by target_token anyway.
            _ = target_dt  # unused when early-exit disabled
        return matched
...
def compare_bart_vs_kalshi(*, date: Optional[str], verbose: bool = False) -> tuple[str, list[GameComparison]]:
    ...
    if verbose:
        date_token = _date_token_from_yyyymmdd(used_date)
        for kind in series:
...
def _is_always_include_game(row: GameComparison) -> bool:
    """True if either team is Hawaii or Northern Iowa (always include in summary)."""
    away_can = canonical_team_name(row.game.away_team)
    home_can = canonical_team_name(row.game.home_team)
    if "hawaii" in away_can or "hawaii" in home_can:
        return True
    if "northerniowa" in away_can or "northerniowa" in home_can:
        return True
    return False
...
def _write_markdown_summary(path: str, date: str, rows: list[GameComparison], *, output_tz: TzInfo, no_ttq_filter: bool = False) -> None:
    out_path = Path(path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--verbose", "-v", action="store_true", help="Print Kalshi event counts for the date (for debugging)")
    p.add_argument("--no-ttq-filter", action="store_true", help="Include all games with spread/total discrepancy (no TTQ > 50 requirement)")
...
def main(argv: list[str]) -> int:
    ...
    date, rows = compare_bart_vs_kalshi(date=args.date, verbose=args.verbose)
    _print_table(date, rows, output_tz=output_tz)
    if args.csv:
        _write_csv(args.csv, date, rows, output_tz=output_tz)
        print(f"Wrote CSV: {args.csv}")
    md_path = args.md or f"reports/bart/bart_kalshi_{date}_summary.md"
    _write_markdown_summary(md_path, date, rows, output_tz=output_tz, no_ttq_filter=args.no_ttq_filter)
